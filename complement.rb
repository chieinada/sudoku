...
class Suite
    ...
    def merge!(other)#other: Suite(Pair)
        other.cells.each {|cell| @cells << cell if !@cells.include?(cell)}
        other.nums.each {|num| @nums << num if !@nums.include?(num)}
        @cells.sort!
        @nums.sort!
        return self
    end
    def merge(other)#other: Pair/Trio
        ...
        other.cells.each {|cell| copy.cells << cell if !copy.cells.include?(cell)}
        other.nums.each {|num| copy.nums << num if !copy.nums.include?(num)}
        copy.cells.sort!
        copy.nums.sort!
        return copy
    end
